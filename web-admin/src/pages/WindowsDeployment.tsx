...
const WindowsDeployment: React.FC = () => {
  ...
  const discoverDevices = async () => {
    try {
      setLoading(true);
      const discoveredDevices = await api('/api/windows-deployment/discover-devices');
  ...
  const loadDeployments = async () => {
    try {
      const deploymentHistory = await api('/api/windows-deployment/deployments');
  ...
  const generateScript = async (config: DeploymentConfig) => {
    try {
      const response = await api('/api/windows-deployment/generate-script', {
        method: 'POST',
  ...
  const executeDeployment = async (config: DeploymentConfig) => {
    try {
      const response = await api('/api/windows-deployment/deploy-bulk', {
        method: 'POST',
        body: JSON.stringify({
...
export default WindowsDeployment;